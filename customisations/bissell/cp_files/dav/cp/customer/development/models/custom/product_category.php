 ...
 */
class product_category extends \RightNow\Models\Base {
    const MAX_LEVEL = 2;
    ...
    public function getDirectDescendants($filterType, $id = null)
    {
        ...
        try {
            ...
            $firstLevelObjects = Connect\ROQL::query("SELECT ID, LookupName FROM Service{$connectName} WHERE Parent = ".$id." ORDER BY DisplayOrder")->next();
    ...
    */
     ...
     */
    public function getChain($id, $level, $ignoreVisibility = true) {
        if(stripos($id, "u") === 0) {
    ...
    private function getTopLevelObjects($connectName, $limit, array $topLevelIDs = array()) {
        ...
        $query = "SELECT Service{$connectName} FROM Service{$connectName} WHERE Parent IS NULL" .
            ...
            " ORDER BY DisplayOrder LIMIT $limit";

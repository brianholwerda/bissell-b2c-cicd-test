<rn:meta title="#rn:msg:SHP_TITLE_HDG#" template="iframe.php" clickstream="KBSearch"/>

<rn:container report_id="101327">
  <form onsubmit="return false;">
    <rn:widget path="search/EventControlledKeywordText" set_value_events='evt_SetKeywordTextValue' label_text="" initial_focus="false"/>
    <rn:widget path="search/EventControlledSearchButton" start_search_events='evt_StartSearch' force_ajax_request='true' force_page_flip="false"/>
  </form>
  <div>
    <rn:widget path="reports/ResultInfo" show_no_results_msg_without_search_term="true"/>
    <rn:widget path="custom/quiq/QuiqMultiline"
      limit_column_lengths='[-1,-1,250]'
      append_links_to_message='true'
      show_send_full_button='true'
      links_regex_to_send='http(s)?:\/\/bissell-ext\.custhelp\.com\/app\/answers\/detail\/a_id\/[0-9]+'
      render_and_hide_column_indexes='4'
      message_column_index='2'
      send_message_column_index='-1'
      hide_columns="answers.updated,answers.a_id"
      truncate_size="100000"/>
...
Centricient.init = function(host) {
  if (!host) {
    throw new Error('Init needs to be called with the hostname of the site that will run your extension');
  }
...
/*
*
* END OF Centricient Messaging Script
*
*/
var url_target = RightNow.Url.getParameter('targetOrigin');

Centricient.init( (url_target) ? decodeURIComponent(url_target) : 'https://bissell.goquiq.com');
Centricient.on('messageReceived', function(data){
  RightNow.Event.fire('evt_SetKeywordTextValue', data.message.text);
  RightNow.Event.fire('evt_StartSearch');
});

RightNow.Event.subscribe('quiq_SendMessage', function(event, data){
  Centricient.prepareMessage(data[0]);
})

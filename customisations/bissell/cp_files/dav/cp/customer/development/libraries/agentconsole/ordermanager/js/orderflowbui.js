...
function setFieldVisibility(){
					...
					if(checkNoCharge != 0){
						...
						if(intval != null){
							...
							if(linktoorder){
								...
								if(rows.length > 0){
									showNoChargeReasonDiv = true;
									showOrderLinkSummaryRowDiv = true;
									showOrderSubmitFormDiv = true;
									showAllOrderDetailsDiv = true;
								} else {
									showNoChargeReasonDiv = true;
	...
	if(showNoChargeReasonDiv){
		$('#divNoChargeReason').css("display","");
	} else {
		$('#divNoChargeReason').css("display","none");
	}
	...
	if(showOrderSubmitFormDiv){
		$('#divOrderSubmitForm').css("display","");
	} else {
		$('#divOrderSubmitForm').css("display","none");
	}
	
	if(showAllOrderDetailsDiv){
		$('#divAllOrderDetails').css("display","");
	} else {
		$('#divAllOrderDetails').css("display","none");
	}
				...
				function createOrderLock(ebsLineID, origOrderNum, newOrderNum, partNum) {
					...
					$.ajax({
						...
						error: function(error){
							console.log('createOrderLock Error: ' + error);
						}

			...
			'Incident.C$Brand'
				...
				$(document).ready(function () {
					...
					$('#paymentmethod').change(function (e,showModal) {
						...
						$('#launchpaymetric').addClass('hidden');
						$('#setpapercheckdetails').addClass('hidden');

						//Display the credit card payment dialog
						if (payMethodText == 'Credit Card') {
							$('#paymetricmodal').modal('show');
							$("#loadingoverlay").fadeIn();
							
							if(!paymetricData.MerchantGUID || !paymetricData.BaseURL){
								RetrievePaymetricConfig();
							}
							
							GenerateAccessToken();
							$('#launchpaymetric').removeClass('hidden');
						} 
					...
					$('#launchpaymetric').click(function () {
						$('#paymetricmodal').modal('show');
						$("#loadingoverlay").fadeIn();
						if(!paymetricData.MerchantGUID || !paymetricData.BaseURL){
							RetrievePaymetricConfig();
					...
					$("#freeshipping").change(function () {
						...
						{
							$('#shippingcost').removeAttr("disabled").trigger("focusout");
						...
						$.ajax({
							...
							success: function(taxResponse) {
								if(taxResponse.SUCCESS) {
									...
									console.log("DEREK TAX RESULT FAILED SHOW ERROR");
								}
							}
						});
					});

...
var ORDER_TYPE_REPAIR_ID = 279;
var ORDER_TYPE_RECALL_ID = 285;
var ORDER_TYPE_STEAMSHOTRECALL_ID = 294;
var RECALL_RETURN_TYPE_ID = 286;
var STEAMSHOTRECALL_RETURN_TYPE_ID = 293;
var RA_RECALL_REPLACE_REASON_ID = 24;
var RA_RECALL_RETURN_REASON_ID = 23;
var RA_RECALL_SHIPMENT_KIT_REASON_ID = 22;
...
var RET_LOCATION_GR_RECALL_RA = 31;
var RET_INSPECTION_ID = 3;
var RET_REPAIR_ID = 280;
...
ORACLE_SERVICE_CLOUD.extension_loader.load("Bissell Order" , "1.0").then(function(extensionProvider) {
	extensionProvider.registerWorkspaceExtension(function(WorkspaceRecord) {
		...
		'Incident.Status.Id',
		'Incident.Repair$RepairStatus',
		'Incident.Repair$RepairComments',
		'Incident.CatId',
		'Incident.AssetId',
		'Incident.RefNo'
			...
			orderHeader.raLinkToOrder = fieldDetails.getField('BUS$OrderHeader.RALinkToOrder').getValue();
			...
			contact.Mobile = fieldDetails.getField('Contact.PhMobile').getValue(); // Nayan
			...
			incident.incidenttype = fieldDetails.getField('Incident.C$Incident_Type').getLabel();
			incident.incidentcategory = fieldDetails.getField('Incident.CatId').getValue();
			incident.incidentasset = fieldDetails.getField('Incident.AssetId').getValue();
			incident.repaircomments = fieldDetails.getField('Incident.Repair$RepairComments').getValue();
			...
			$.when(defCountryListLoaded, defProvinceListLoaded, defProvinceAbbreviationListLoaded).done(function () {
				...
				buildOrderReturnTypeList();
				setWorkspaceValues();
				buildShipMethodList();
				getReasonList_FULL();
				getActionList_FULL();
				getProductRecallList();
				buildRecallLineItems();
				
				if(incident.incidentasset != null){
					var assetdetails = getAssetDetails(incident.incidentasset);
					if(assetdetails.SerialNumber){
						var serialNumVal = assetdetails.SerialNumber.replace(/\t/g, " ");
						serialNumVal = serialNumVal.replaceAll(" ", "");
						$('#serialnumber').val(serialNumVal);
					}
					if(assetdetails.LookupName){
						$('#modelnumber').val(assetdetails.LookupName);
					}	
				}
			});

			$.when(defActionCodeListLoaded,defReasonCodeListLoaded,defProductRecallListLoaded,defRecallLinesBuilt).done(function () {
			...
			$(document).ready(function () {
				...
				$('#partnumber').focusout(function () {
					...
					else
					{
						...
						var repairReturn = getSelectedValue('returntype') == RET_REPAIR_ID ? '&repairreturn=true' : '';
						
						$.ajax({
							type: "GET",
							url: CUSTOM_SCRIPTS_PATH + 'partsearch.php?sid=' + sid + '&_=' + date.getTime() + '&itemnumber=' + itemNumber + '&country=' + shippingAddress.CountryCodeId + retSearch + repairReturn,
							cache: false,
				...
				function setSelectedPart(selectedData) {
					const recall = window.productRecallList.filter(obj => obj.product.indexOf(selectedData.name.toUpperCase() + " ") > -1);
					
					if(recall.length > 0 && [ORDER_TYPE_RECALL_ID, ORDER_TYPE_STEAMSHOTRECALL_ID].indexOf(orderHeader.orderTypeID) == -1 && addPartType == 2){
						$('#btnAddLine').attr("disabled","disabled");
						$('#partnumber').val('');
						alert('Model ' + selectedData.name + ' is under a product recall and cannot be returned using a standard RA at this time.\n\nPlease follow recall process to further assist this consumer.');
					} else if (recall.length == 0 && [ORDER_TYPE_RECALL_ID, ORDER_TYPE_STEAMSHOTRECALL_ID].indexOf(orderHeader.orderTypeID) > -1 && addPartType == 2) {
						$('#btnAddLine').attr("disabled","disabled");
						$('#partnumber').val('');
						alert('This model number is not under a product recall. The Return Item on a recall RA may only be used for model numbers effected by the product recall.');
					} else if(window.ReturnPartsTable.rows().count() > 0 && orderHeader.orderTypeID == ORDER_TYPE_RECALL_ID && addPartType == 2) {
						$('#btnAddLine').attr("disabled","disabled");
						$('#partnumber').val('');
						alert('A Return Part already exists on this RA. Only one Return Part may be added for a Product Recall RA.');
					} else if(window.ReplacementPartsTable.rows().count() > 1 && orderHeader.orderTypeID == ORDER_TYPE_RECALL_ID && addPartType == 3) {
						$('#btnAddLine').attr("disabled","disabled");
						$('#partnumber').val('');
						alert('Unable to add an additional replacement part. The product recall RA may only contain one line for the shipping kit and one replacement model number.');
					} else if(orderHeader.orderTypeID == ORDER_TYPE_RECALL_ID && addPartType == 3 && getSelectedText('actioncode') == 'Return for Credit') {
						$('#btnAddLine').attr("disabled","disabled");
						$('#partnumber').val('');
						alert('Cannot add an additional replacement item when using the Return for Credit Action Code.');
					} else if(selectedData.name.toUpperCase() == RET_REPAIR_SKU){
						$('#btnAddLine').attr("disabled","disabled");
						$('#partnumber').val('');
						alert('SKU ' + RET_REPAIR_SKU + ' is no longer valid. Please select a different Part Number.');
					} else if(orderHeader.orderTypeID == ORDER_TYPE_REPAIR_ID && selectedData.repaireligible.toUpperCase() != 'YES'){
						$('#btnAddLine').attr("disabled","disabled");
						$('#partnumber').val('');
						alert('SKU is not enabled for Repair. You may only add Repair Enabled SKUs to a Repair RA.');
					} else {
						...
						if(recall.length > 0 && orderHeader.orderTypeID == ORDER_TYPE_RECALL_ID && addPartType == '2') {
							$('#returntype').val(RECALL_RETURN_TYPE_ID).trigger('change');
						} else if (recall.length > 0 && orderHeader.orderTypeID == ORDER_TYPE_STEAMSHOTRECALL_ID && addPartType == '2') {
							$('#returntype').val(STEAMSHOTRECALL_RETURN_TYPE_ID).trigger('change');
						} else if (orderHeader.orderTypeID == ORDER_TYPE_RECALL_ID && addPartType == '3'){
							$('#returntype').val(RECALL_RETURN_TYPE_ID);
							$('#reasoncode').val(RA_RECALL_REPLACE_REASON_ID).trigger('change');
						}
					}
				}
				...
				$('#returntype').change(function () {
					...
					if(getSelectedText('returntype') == "Product Recall"){
						if(addPartType == "2"){
							setTimeout(function () {
								buildReasonList(returnType, RA_RECALL_RETURN_REASON_ID, true);
							}, 100);
							$('#reasoncode').trigger('change');
						} else if (addPartType == "3") {
							setTimeout(function () {
								buildReasonList(returnType, RA_RECALL_REPLACE_REASON_ID, true);
							}, 100);
							$('#reasoncode').trigger('change');
						} else {
							alert('Invalid ine type detected');
						}
					} else {
				...
				$('#actioncode').change(function () {
					if (window.ReturnPartsTable.rows().count() > 0) {
						...
						alert('Must remove existing items under Return Parts before changing Action Code value');
						return false;
					}
				...
				$('#ralinktoorder').change(function () {
					...
					WorkspaceRecord.updateField('BUS$OrderHeader.RALinkToOrder', checkNoCharge); 
					orderHeader.raLinkToOrder = checkNoCharge;
				});
				...
				$("#btnAddLine").click(function () {
					...
					const recall = window.productRecallList.filter(obj => obj.product.indexOf($('#partnumber').val().toUpperCase() + " ") > -1);
					...
					if(getSelectedText('returntype') == "Product Recall" && orderHeader.orderTypeID != ORDER_TYPE_RECALL_ID){
						alert("The 'Product Recall' return type may only be used with a Recall RA");
						return;
					}
					
					if(getSelectedText('returntype') == "SteamShot Product Recall" && orderHeader.orderTypeID != ORDER_TYPE_STEAMSHOTRECALL_ID){
						alert("The 'SteamShot Product Recall' return type may only be used with a SteamShot Recall RA");
						return;
					}
					
					if(getSelectedText('returntype') != "Product Recall" && orderHeader.orderTypeID == ORDER_TYPE_RECALL_ID){
						alert("You must use the Return Type 'Product Recall' when working with a Recall RA");
						return;
					}
					
					if(getSelectedText('returntype') != "SteamShot Product Recall" && orderHeader.orderTypeID == ORDER_TYPE_STEAMSHOTRECALL_ID){
						alert("You must use the Return Type 'SteamShot Product Recall' when working with a SteamShot Recall RA");
						return;
					}
					
					if((getSelectedText('returntype') == "Product Recall" || getSelectedText('returntype') == "SteamShot Product Recall") && addPartType == "2" && recall.length == 0){
						alert("This product is not part of the Product Recall. May only select items under Recall when using the Product Recall Return Type.");
						return;
					}
					
					if(getSelectedText('returntype') != "Product Recall" && addPartType == "2" && recall.length > 0 && recall.recall == "2022 Crosswave Battery"){
						alert("This product is under a Product Recall. You may only use the Return Type of 'Product Recall' for this item");
						return;
					}
					
					if(getSelectedText('returntype') != "SteamShot Product Recall" && addPartType == "2" && recall.length > 0 && recall.recall == "2024 SteamShot"){
						alert("This product is under a Product Recall. You may only use the Return Type of 'SteamShot Product Recall' for this item");
						return;
					}
					
					if(getSelectedText('returntype') == "Product Recall" && addPartType == "2" && getSelectedText('reasoncode') != "CWA Return Line"){
						alert("Must use Reason Code 'CWA Return Line' when adding return parts for a Product Recall");
						return;
					}
					
					if(getSelectedText('returntype') == "Product Recall" && addPartType == "3" && getSelectedText('reasoncode') != "CWA Replace Line"){
						alert("Must use Reason Code 'CWA Replace Line' when adding replacement parts for a Product Recall");
						return;
					}
					
					if (getSelectedText('returntype') == "Product Recall" && addPartType == "2" && incident.incidenttype == "Claim" && getSelectedText('actioncode') != "Examine and Report"){
						alert("Must use the Action Code 'Examine and Report' for Product Recall Claim RA's");
						return;
					}
					
					if (getSelectedText('returntype') == "Product Recall" && addPartType == "2" && incident.incidenttype != "Claim" && getSelectedText('actioncode') == "Examine and Report"){
						alert("The 'Examine and Report' Action Code may only be used for Product Recall Claim RA's.");
						return;
					}
					
					if(getSelectedValue('returnlocation') == RET_LOCATION_GR_RECALL_RA && incident.incidenttype != "Claim"){
						alert('The return location of Grand Rapids may only be used for Claim incidents.');
						return;
					}
					
					if(getSelectedText('returntype') == "Repair" && addPartType == "2"){
						var returnRow = window.ReturnPartsTable.rows().data();
						if(returnRow.length > 0){
							alert('May not add more than one Return Row to a Repair RA.');
							return;
						}
					}
					
					if(getSelectedText('returntype') == "Repair" && addPartType == "3"){
						var replacementRow = window.ReplacementPartsTable.rows().data();
						if(replacementRow.length > 0){
							alert('May not add more than one Replacement Row to a Repair RA.');
							return;
						}
						
						var returnRow = window.ReturnPartsTable.rows().data();
						if(returnRow.length == 0){
							alert('Must add Return Parts before Replacement when creating a Repair RA.');
							return;
						}
						
						if(returnRow[0].part.toUpperCase() != $('#partnumber').val().toUpperCase()){
							alert('For Repair RAs, Return and Replace Part Numbers must be use the same SKU.');
							return;
						}
					}
					...
					$.blockUI({
						...
						onBlock: function () {
							...
							//if (confirm('Are you sure you would like to submit this RA?')) {
								...
								orderSubmitObj.order.RAOrderDate = new Date(Date.now()).toISOString().split('T')[0].replaceAll("-","");
								
								orderSubmitObj.order.IncidentCategory = incident.incidentcategory;
								orderSubmitObj.order.IncidentAsset = incident.incidentasset;
								orderSubmitObj.order.Asset = getAssetDetails(incident.incidentasset);
								orderSubmitObj.order.Category = getCategoryText(incident.incidentcategory);
								
								if(incident.repaircomments){
									orderSubmitObj.order.RepairNotes = incident.repaircomments.replace(/\r/g, " ").replace(/\n/g, " ").replace(/\t/g, " ").replace(/"/g, "'");
								} else {
									orderSubmitObj.order.RepairNotes = "";
								}
			...
			function setWorkspaceValues(skipOHUpdate) {
				...
				if (!skipOHUpdate){
					$('#ralinktoorder option[value="' + orderHeader.raLinkToOrder + '"]').attr('selected', true).trigger('change');
			...
			function AutoSelectShippingMethod() {
				...
				if($('#shippingmethod').val()){
					setShipMethod($('#shippingmethod').val(), true);
				} else {
					setShipMethod("", true);
				}
			...
			function getLineItemsForOrderProcessing() {
				...
				if (rows.length > 0) {
					...
					for (var i = 0; i < rows.length; i++) {
						...
						orderLineItem.Warehouse = getSelectedDataAttribute('returnlocation', 'code');
				...
				if (rows.length > 0) {
					...
					for (var i = 0; i < rows.length; i++) {
						...
						if(getSelectedValue('returntype') == RET_REPAIR_ID){
							//orderLineItem.Warehouse = row.returnlocation;
							orderLineItem.Warehouse = getSelectedDataAttribute('returnlocation', 'code');
						} else {
							orderLineItem.Warehouse = null;
						}
			...
			// GET Billing/Shipping address for the return submission
			...
			function setBillToAddressFromContact()
			{
				...
				{
					WorkspaceRecord.updateField('BUS$OrderHeader.BillToPhone', contact.Phone);
				}
				else
				{
					WorkspaceRecord.updateField('BUS$OrderHeader.BillToPhone', contact.Mobile);
				}
				//Nayan Stop
				
				//WorkspaceRecord.updateField('BUS$OrderHeader.BillToPhone', contact.Phone); // Nayan
				...
				if(contact.Mobile == null)
				{
					billingAddress.Phone = contact.Phone;
				}
				else
				{
					billingAddress.Phone = contact.Mobile;
				}
				//Nayan Stop
				
				//billingAddress.Phone = contact.Phone; //Nayan 
			...
			function setShipToAddressFromContact()
			{
				...
				if(contact.Mobile == null)
				{
					WorkspaceRecord.updateField('BUS$OrderHeader.ShipToPhone', contact.Phone);
				}
				else
				{
					WorkspaceRecord.updateField('BUS$OrderHeader.ShipToPhone', contact.Mobile);
				}
				...
				{
					shippingAddress.Phone = contact.Mobile; 
				}
				//Nayan Stop
				
				
				//shippingAddress.Phone = contact.Phone; //Nayan
			...
			function orderSubmitSuccessCallback(data) {
				...
				// CHECK if a blank RA was submitted
				if(data.ResponseMessage.toLowerCase() === 'blank ra submission')
				{
					alert("Blank RA Submission detected, please try to submit again. If the Blank RA submission detection continues, please Save/Close the RA, reopen the RA, and try the submission again. You do not need to create a new RA or a new incident.");
					$.unblockUI();
					return;
				}
				...
				if (data.ResponseCode.toLowerCase() === 'success') {
					...
					if(hasValue(data.FedEx)) {
						...
						if (data.FedEx.FedExResponse == 'SUCCESS' || data.FedEx.FedExResponse == 'WARNING' || data.FedEx.FedExResponse == 'NOTE'){
					...
					if(orderHeader.orderTypeID == ORDER_TYPE_REPAIR_ID){
						WorkspaceRecord.updateField('Incident.Status.Id', 124);
						WorkspaceRecord.updateField('Incident.Repair$RepairStatus', 275);
						WorkspaceRecord.updateField('Incident.C$SendMsgRepairIns', true);
					} else if (orderHeader.orderTypeID == ORDER_TYPE_STEAMSHOTRECALL_ID){
						WorkspaceRecord.updateField('Incident.C$ActionButtonsTrigger', true);
						WorkspaceRecord.updateField('Incident.C$RaProcessedSendEmail', true);
					}
					...
					$('#tblLinkedOrderData tbody tr').each(function() {
						...
						updateOrderLock(lineIDText, orderHeader.returnAuthorizationNumber);
					});
			...
			function isOrderValid() {
				...
				if ((orderHeader.orderTypeID != ORDER_TYPE_RA_ID) && (orderHeader.orderTypeID != ORDER_TYPE_REPAIR_ID) && (orderHeader.orderTypeID != ORDER_TYPE_RECALL_ID) && (orderHeader.orderTypeID != ORDER_TYPE_STEAMSHOTRECALL_ID)) {
				...
				if(getSelectedDataAttribute('returnmethod', 'provider') == "FedEx"){
					var returnRow = window.ReturnPartsTable.rows().data();
					if (returnRow.length > 0) {
						for (var i = 0; i < returnRow.length; i++) {
							var row = returnRow[i];
							const recall = window.productRecallList.filter(obj => obj.product.indexOf(row.part.toUpperCase() + " ") > -1);
							if(recall.length > 0){
								message = message + 'Return Items are under a product recall and cannot be returned using a FedEx Return Method.\n\n';
								isvalid = false;
							}
						}
					}
				...
				if(ORDER_TYPE_RECALL_ID == orderHeader.orderTypeID){
					if(getSelectedDataAttribute('returnmethod', 'provider') != "UPS"){
						message = message + 'Must use a UPS Return Method for a Recall RA.\n\n';
						isvalid = false;
					}
					
					var shipKitIncluded = false;
					var replacementRow = window.ReplacementPartsTable.rows().data();
					if (replacementRow.length > 0) {
						for (var i = 0; i < replacementRow.length; i++) {
							var row = replacementRow[i];
							if(row.part.toUpperCase() == '1635561'){
								shipKitIncluded = true;
								
							}
						}
						if(replacementRow.length == 1 && getSelectedText('actioncode') == 'Replacement Upon Return'){
							message = message + 'A replacement part is required.\n\n';
							isvalid = false;
						}
					}
					...
					if(!shipKitIncluded){
						message = message + 'Must have the Shipment Kit as one of the replacement items on a Recall RA.\n\n';
						isvalid = false;
					}
					
					var returnRow = window.ReturnPartsTable.rows().data();
					if(returnRow.length > 1){
						message = message + 'May only have one Return Item when creating a Recall RA.\n\n';
						isvalid = false;
					}
				}
			...
			function updateOrderHeaderRAFields(){
				var raData = {};
				raData.OrderHeaderId = orderHeaderId;
				raData.RAReturnLocation = getSelectedValue('returnlocation');
				raData.RAReturnType = getSelectedValue('returntype');
				raData.RAReturnMethod = getSelectedValue('returnmethod');
				raData.RAReasonCode = getSelectedValue('reasoncode');
				raData.RAActionCode = getSelectedValue('actioncode');
				raData.NoChargeReason = getSelectedValue('nochargereason');
				raData.RALinkToOrder = getSelectedValue('ralinktoorder');
				
				$.ajax({
					type: "POST",
					url: AJAX_ENDPOINT + '?action=updateorderheaderrafields&sid=' + sid,
					data: raData,
					cache: false,
					success: function (data) {
						console.log('entered updateOrderHeaderRAFields success');
						console.log(data);
					},
					dataType: "json",
					error: function (request, status, error) {
			...
			function disableSubmittedOrder(submissionDisable) {
				if (orderHeader.pageLocked) { 
					...
					$("#divSubmitSummary").html('RA created successfully. The Return # is ' + orderHeader.returnAuthorizationNumber);
				};
				
				if(orderHeader.returnAuthorizationNumber && orderHeader.status == 'Submitted'){
					$("#divSubmitError").css("display","none");
					$("#divSubmitSummary").css("display","");
					$("#divSubmitSummary").html('RA has been successfully processed. The Return # is ' + orderHeader.returnAuthorizationNumber + '.');
				} else if (!orderHeader.returnAuthorizationNumber && (orderHeader.status == 'Submitted' || orderHeader.status == 'Queued')){
					$("#divSubmitSummary").css("display","none");
					$("#divSubmitError").css("display","");
					$("#divSubmitError").html('RA has been successfully processed; however an RA Number is not available at this time. DO NOT RESUBMIT ANOTHER RA FOR THIS CONSUMER!');
				} else if (orderHeader.status == 'Failed'){
					$("#divSubmitSummary").css("display","none");
					$("#divSubmitError").css("display","");
					$("#divSubmitError").html('The RA was not processed due to a system communication error. Apologize to the customer and let the customer know their RA will be saved. Advise them you will call them back as soon as the systems are restored. DO NOT ATTEMPT TO PLACE ANOTHER RA FOR THE CONSUMER!');
				} else if (orderHeader.status == 'FedEx Error'){
					$("#divSubmitSummary").css("display","none");
					$("#divSubmitError").css("display","");
					$("#divSubmitError").html('The RA has been successfully processed, but there was an error generating the FedEx label.');
				} else if (orderHeader.status == 'Cancelled'){
					$("#divSubmitSummary").css("display","none");
					$("#divSubmitError").css("display","");
					$("#divSubmitError").html('This RA has been cancelled.');
				}
			}
			...
			function buildOrderReturnTypeList(reset) {
				var setOptions = function (data) {
					//Append blank options
					var blankOption = "<option value=''>[+]</option>";
					...
					$.each(window.menulists.order_returntypelist, function (index, orderReturnType) {
						if (pageLocked || (!pageLocked && orderReturnType.active == true)){
							var option = $("<option></option>");
							option.attr("value", orderReturnType.id)
							.text(orderReturnType.label)

							$('#returntype').append(option);
						}
					});

					if (hasValue(orderHeader.returnType)) {
						$('#returntype option[value="' + orderHeader.returnType + '"]')
							.attr('selected', true);
							//.trigger('change',true);
				...
				if(reset === true)
					setOptions(window.menulists.order_returntypelist);
				else
				{
					$.ajax({
						type: "GET",
						url: AJAX_ENDPOINT + '?action=getrareturntypelist&country=' + shippingAddress.CountryCodeId + '&incidenttype=' + incident.incidenttype + '&sid=' + sid,
						cache: true,
						success: function (data) {
							if (window.menulists == undefined)
								window.menulists = {};
							data.sort(GetSortOrder("displayOrder"));
							window.menulists.order_returntypelist = data;
							setOptions(data);
						},
						error: function (error) {
							console.log('Enter Return Type Error');
							console.log(error);
			...
			function buildOrderNoChargeReasonList(reset) {
				var setOptions = function (data) {
					...
					$.each(window.menulists.order_nochargereasons, function (index, orderNoChargeReason) {
						if(orderNoChargeReason.ordertype == "RA"){
			...
			function getProductRecallList() {
				$.ajax({
					type: "GET",
					url: AJAX_ENDPOINT + '?action=getproductrecalls&country=' + shippingAddress.CountryCodeId + '&sid=' + sid,
					cache: false,
					success: function (data) {
						window.productRecallList = data;
						defProductRecallListLoaded.resolve();
					},
					error: function (xhr, status, error) {
						console.log('getProductRecallList bissell interface Error');
						console.log(error);
					},
					dataType: "json"
				});
			...
			function buildRecallLineItems(){
				if(orderHeader.orderTypeID == ORDER_TYPE_RECALL_ID && !orderHeader.pageLocked){
					$.ajax({
						type: "GET",
						url: AJAX_ENDPOINT + '?action=getorderlinedata&linetype=3&orderheaderid=' + getParameterByName('oid') + '&sid=' + sid,
						cache: false,
						success: function (data) {
							if(data.aaData.length == 0){
								buildRecallLineItemsCallback();
							} else {
								defRecallLinesBuilt.resolve();
							}
						},
						dataType: "json",
						error: function (request, status, error) {
			...
			function buildRecallLineItemsCallback(){
				var orderLine = {};
				orderLine.OrderHeaderId = orderHeaderId;
				orderLine.Status = "Entered";
				orderLine.PartNumber = "1635561";
				orderLine.PartDescription = "Shipping Kit";
				orderLine.Quantity = "1";
				orderLine.UnitPrice = "0.00";
				orderLine.AdjustedPrice = "0.00";
				orderLine.LineType = 3;
				orderLine.Line = 1;
				orderLine.NoCharge = true;
				orderLine.ReasonCode = RA_RECALL_SHIPMENT_KIT_REASON_ID;
			...
			function getAssetDetails(assetID) {
				...
				$.ajax({
					type: "GET",
					url: AJAX_ENDPOINT + '?action=getassetdetails&assetid=' + assetID + '&sid=' + sid,
					async: false,
					success: function (data) {
						console.log('Enter getAssetDetails Success');
						console.log(data);
						response.AssetID = data.AssetID;
						response.LookupName = data.LookupName;
						response.SerialNumber = data.SerialNumber;
					},
					error: function (error) {
						console.log('Enter getAssetDetails Error');
						console.log(error);
						// window.requireorderflow = false;
						// setFieldVisibility();
					},
			...
			function getCategoryText(catID) {
				...
				$.ajax({
					type: "GET",
					url: AJAX_ENDPOINT + '?action=getcategorydetails&categoryid=' + catID + '&sid=' + sid,
					async: false,
					success: function (data) {
						console.log('Enter getCategoryText Success');
						console.log(data);
						for (var i = data.length - 1; i >= 0; i--){
							if(i == 0){
								response += data[i].lookupname;
							} else {
								response += data[i].lookupname + ' -> ';
							}
						}
					...
					error: function (error) {
						console.log('Enter getCategoryText Error');
						console.log(error);
						// window.requireorderflow = false;
						// setFieldVisibility();
					},

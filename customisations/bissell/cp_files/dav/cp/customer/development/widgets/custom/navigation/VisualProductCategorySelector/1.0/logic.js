RightNow.namespace('Custom.Widgets.navigation.VisualProductCategorySelector');
Custom.Widgets.navigation.VisualProductCategorySelector = RightNow.Widgets.extend({ 
    /**
     * Widget constructor.
     */
    constructor: function() {
        ...
        if (this.widgetElement) {
            ...
            this._attachEventHandlers(this.widgetElement, [
                ...
                ['click', 'a.rn_ShowChildrenImage', this._onShowChildrenImageClick],
    ...
    _onShowChildrenClick: function(e) {
        e.halt();
		//console.log("inside _onShowChildrenClick");
        var target = e.target,
     ...
     * @param {Object} e Click event
     */
    _onShowChildrenImageClick: function(e) {
        e.halt();
		//console.log("inside _onShowChildrenImageClick");
        var target = e.target,
            id = parseInt(target.getAttribute('data-id'), 10),
            el = target.ancestor('.rn_ItemGroup'),
            label = this.Y.Lang.trim(target.ancestor('.rn_VisualItemContainer > .rn_ImageContainer').one('.rn_ItemLink').get('text'));
    ...
    _showChildren: function(parent) {
		...
		//console.log(subItemsAlreadyRendered);
		this.widgetElement.all('.rn_ItemGroup.rn_BaseGroup .rn_Item').removeClass('rn_ProdSelected');
		this.widgetElement.all('.rn_ItemGroup.rn_BaseGroup .rn_Item .rn_VisualItemContainer .rn_ImageContainer').removeClass('rn_ProdSelected');
		
		myClassName = "li.rn_Item.rn_ItemWithID" + parent.id;
		this.widgetElement.all(myClassName).addClass('rn_ProdSelected');
		
		myClassName = "li.rn_Item.rn_ItemWithID" + parent.id + ' .rn_VisualItemContainer .rn_ImageContainer';
		//alert(myClassName);
		this.widgetElement.all(myClassName).addClass('rn_ProdSelected');
		
        if (subItemsAlreadyRendered) {
	        //console.log("inside subItemsAlreadyRendered");
	        ...
	        //console.log("inside requestchildre");
    ...
    _showNewGroup: function(items, options) {
        ...
        this._hide(this.widgetElement.all('.rn_ItemGroup.rn_SubGroup'));
        ...
        if (options.focusOnFirst) {
            //this._focusFirstLink(newItemGroup);  KLA - TURNED OFF PER REQUEST FROM STEVE BOWKER, WILL TURN BACK ON IF ADA ISSUES.
        }
    },
    ...
    _renderPagedPagination: function(pageNumber, itemsPerPage, itemsCount, forwardButton) {
        ...
        if(pageCount > 1) {
            ...
            for (var i = 1; i < pageCount + 1; i++) {
                var pageLinkListItem = this.Y.Node.create('<li class="rn_ItemPaginationNumber"><a href="javascript:void(0);">' + i + '</a></li>'),
    ...
    _goToPreviousLevel: function(e) {
	    //console.log("inside _goToPreviousLevel");
    ...
    _showSubItemsFor: function(id) {
	    //console.log("inside _showSubItemsFor");
        ...
        if (existing) {
            ...
            this.widgetElement.all('.rn_ItemGroup.rn_SubGroup').addClass('rn_Hidden');

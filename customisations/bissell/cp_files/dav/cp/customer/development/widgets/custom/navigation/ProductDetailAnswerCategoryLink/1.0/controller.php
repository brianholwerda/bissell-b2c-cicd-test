...
class ProductDetailAnswerCategoryLink extends \RightNow\Libraries\Widget\Base {
    ...
    function getData() {
        ...
        if($topics->errors[0]->internalMessage) {
            ...
            foreach($topics->result as $topicIndex => $topicItem):
                    $answerContent[$topicItem['id']] = $this->CI->model('Answer')->getPopular($searchLimit, $pId, $topicItem['id']);
            ...
            foreach($answerContent as $answerResultIndex => $answerResultGroup):
				...
				$validProduct = (strpos($answerResultGroup->errors[0]->externalMessage, 'Invalid') !== false);
				//print_r($validProduct);

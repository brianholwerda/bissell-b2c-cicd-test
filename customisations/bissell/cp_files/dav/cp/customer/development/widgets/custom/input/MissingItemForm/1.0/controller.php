...
class MissingItemForm extends \RightNow\Libraries\Widget\Base {
    ...
    function getData() {
		...
		if($myCountry == "")
			$myCountry = 1;
		$this->data['attrs']['countryID'] = $myCountry;
		//print_r($Contact->Emails[0]);
		$this->data['attrs']['emailAddr'] = $Contact->Emails[0]->Address;
		$this->data['attrs']['ShippingPhone'] = "";
		for ($x = 0; $x < count($Contact->Phones); $x++) {
			if($Contact->Phones[$x]->Number != "")	{
				$this->data['attrs']['ShippingPhone'] = $Contact->Phones[$x]->Number;
				break;
			}
		}
		...
		//Bissell does not want to show last 4 of US zip
		if(strpos($newzip, '-') > 0)
			$newzip = substr($newzip, 0, strpos($newzip, '-'));
		...
		$reservableInventoryLimit = Config::getConfig(CUSTOM_CFG_WDMR_INVENTORY_LIMIT);
		if($this->data['attrs']['reservableInventory'] > $reservableInventoryLimit ) {
			$this->data['attrs']['OutOfStock'] = 0;
		} else {
			$this->data['attrs']['OutOfStock'] = 1;
		}
		$this->data['attrs']['productPriceCurrent'] = floatval($getProdSkuPrice['Price']);
		...
		} else if($getProdSkuPart['HazardCode'] != '-' && $getProdSkuPart['HazardCode'] != '') {
			//echo("is hazardous");
			$this->data['attrs']['isHazardous'] = '1';
		} else {
			//echo("is NOT hazardous");

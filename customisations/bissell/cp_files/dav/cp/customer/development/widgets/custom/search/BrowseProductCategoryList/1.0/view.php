			...
			<rn:block id="preItem"/>
				<div class="rn_HierList rn_HierList_<?=$key;?>"   style="float:left;width:<?=$colWidth;?>%;" >
					<h3 id="rn_ProductHeader_<?=$value['hierList'];?>" style="padding-bottom:10px;" >
						<? 
							//look for image file with same name as the product ID found under standard theme images.  If exists then display it
							$prodImageName = "product_id_" . $value['hierList'] . ".png";
							$prodImageFile = HTMLROOT . "/euf/assets/themes/standard/images/" . $prodImageName;
							$prodImageUrl = "/euf/assets/themes/standard/images/" . $prodImageName;
							$imageHTML = "";
							if(file_exists ($prodImageFile)) 
							{	 
								$imageHTML = "<img src='$prodImageUrl' alt='" . $value['label'] . " Icon Image' style='padding-bottom:10px;'>";
							}
		
						?>
						<a href="<?=$this->data['attrs']['report_page_url'] . $this->data['appendedParameters'] . \RightNow\Utils\Url::sessionParameter() . "/{$this->data['type']}/" . $value['hierList']; ?>"><?=$imageHTML;?><br /><span class="rn_ProductHeader"><?=htmlspecialchars($value['label'], ENT_QUOTES, 'UTF-8');?></span></a>
					</h3>
                    <div class="catBox">
                    <!-- Sub-Categories -->

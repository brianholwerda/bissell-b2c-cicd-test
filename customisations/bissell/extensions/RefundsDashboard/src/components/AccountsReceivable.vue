...
const approvedRefundsTotal = computed(() => {
  ...
  return OpenRefunds.value.reduce((sum, refund) => {
    if (rowSelections[refund.refundId] === 'APPROVE') {
...
const OpenRefundsHeaders = [
  ...
  { key: "cCReviewedDate", title: "CC Date Reviewed" },
          ...
          <v-table density="compact">
            ...
            <tbody>
              <tr v-for="result in refundResults" :key="result.refundId"
                  :class="{
                    'result-success-row': result.refundStatus === 'AR_APPROVED_ERP_SUBMIT_SUCCESS' || result.refundStatus === 'AR_DENIED',
                    'result-error-row': result.refundStatus !== 'AR_APPROVED_ERP_SUBMIT_SUCCESS' && result.refundStatus !== 'AR_DENIED'
                  }"
                ...
                <td style="font-weight: bold; letter-spacing: 0.5px;">
                  <span v-if="result.refundStatus === 'AR_APPROVED_ERP_SUBMIT_SUCCESS' || result.refundStatus === 'AR_DENIED'" style="color: #388e3c;">Success</span>
      ...
      <v-card class="summary-card summary-card-total" elevation="2">
        <div class="d-flex align-center pa-3">
          ...
          <div>
            ...
            <div class="text-h5 font-weight-bold text-primary">{{ formatCurrency(approvedRefundsTotal) }}</div>
          </div>
        </div>
      </v-card>
    </v-col>
    ...
    >
      ...
      <template v-slot:body="{ items }">
        <template v-if="items.length === 0">
          <tr>
            <td :colspan="OpenRefundsHeaders.length" class="pa-8 text-center">
              <div class="d-flex flex-column align-center">
                <v-icon size="64" color="success">mdi-check-circle-outline</v-icon>
                <div class="text-h6 mt-4 text-grey">No Refunds Pending Review</div>
                <div class="text-body-2 text-grey-darken-1 mt-2">There are currently no open refunds awaiting A/R Approval.</div>
              </div>
            </td>
          </tr>
        </template>
        <template v-else>
          <tr v-for="item in items" :key="item.refundId" :class="getRowClass(item)" class="refund-row">
            <td>
            ...
            <td>{{ item.pONumber || '' }}</td>
            <td>{{ formatDateET(item.cCReviewedDate) }}</td>
            ...
            <td>
              ...
              <v-icon v-if="missingDeclineComments[item.refundId]" color="error" size="20" class="ml-2 pulse-icon">mdi-alert-circle</v-icon>
            </td>
          </tr>
        </template>
      </template>
...
/* Pulsing animation for missing comment indicator */
@keyframes pulse {
  0%, 100% {
    opacity: 1;
    transform: scale(1);
...
/* Enhanced row-missing-comment with pulsing border */
